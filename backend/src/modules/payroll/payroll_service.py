...
class PayrollService:
    ...
    def get_salary_history(self, employee_id: int):
        ...
        if not data:
            raise HTTPException(status_code=404, detail="No salary history")
    ...
    def export_employee_salary_excel(self, employee_id: int, year: Optional[int] = None):
        ...
        if not data:
            raise HTTPException(status_code=404, detail="No salary data to export")
    ...
    def export_full_employee_report(self, employee_id: int, month: str):
        ...
        if not merged.empty and 'Tháng' in merged.columns:
    ...
    def get_salary_trend(
        ...
        for row in rows:
            month = row['Month']
            net_salary = row['NetSalary'] or 0

            if month not in results:
                results[month] = 0

            results[month] += net_salary
    ...
    def export_all_employees_excel(self, month: str):
        ...
        if not data:
            raise HTTPException(status_code=404, detail="No salary data for this month")
    ...
    def export_by_department_excel(self, month: str, department_name: str):
        ...
        if not data:
            raise HTTPException(status_code=404, detail="No salary data for this department in the month")
