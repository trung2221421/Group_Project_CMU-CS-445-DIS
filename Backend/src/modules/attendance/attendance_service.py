...
class AttendanceService:
    ...
    def _calc_trend(self, rows):
        ...
        for row in rows:
            ...
            if row['EmployeeCount'] > 0 and days > 0:
                percent = round((row['TotalWorkDays'] / (row['EmployeeCount'] * days)) * 100)
            else:
                percent = 0
            result.append({
                'Month': month,
                'Percent': percent,
                'TotalWorkDays': row['TotalWorkDays'],
    ...
    def get_department_attendance_trend(self, months: int = 6, reference_month: str = None, department_name: str = None):
        if reference_month is None:
            reference_month = datetime.now().strftime('%Y-%m')
        data = self.repo.get_department_attendance_trend(months, reference_month, department_name)
        if not data:
            return []
        return self._calc_trend(data)
